...
// source of truth for order + titles, chapters[0] being the owner itself); every other member's
// metadata carries `parentId` pointing back at the owner. db.js keeps a denormalized aggregate
// (chapterCount / aggPages / aggSize) on the owner's stat record for O(1) card rendering.
//
...
// Tags from every absorbed gallery are unioned onto the owner. `opts.title` overrides the default
// chapter title (the child's own gallery title).
export async function mergeIntoSeries(ownerId, childId, opts = {}) {
  ...
  const tagLists = [ownerMeta.tags, childMeta.tags];
  ...
  if (childChapters && childChapters.length > 1) {
    ...
    await mutateGallery(childId, { chapters: null, seriesTitle: '' });
  } else {
    const title = opts.title != null ? opts.title : (pickTitle(childMeta, 'en') || '');
    chapters.push({ id: childId, title });
  }

  const patch = { chapters, tags: unionTags(...tagLists) };
...
async function _writeSeries(oldOwnerId, newChapters) {
  ...
  if (!owner || newChapters.length < 2) {
    if (owner) await mutateGallery(owner, { chapters: null, seriesTitle: '', parentId: null });
    if (oldOwnerId !== owner) {
      await mutateGallery(oldOwnerId, { chapters: null, seriesTitle: '', parentId: null });
  ...
  await mutateGallery(owner, {
    ...
    parentId: null,
    ...(oldOwnerId !== owner ? { tags: prevMeta?.tags } : {}),   // carry the unioned tags along
  });
  ...
  if (oldOwnerId !== owner) {
    ...
    await mutateGallery(oldOwnerId, stillPresent
      ? { chapters: null, seriesTitle: '' }              // demoted to a plain chapter
      : { chapters: null, seriesTitle: '', parentId: null }); // removed entirely → standalone
    await refreshSeriesAggregate(oldOwnerId);
  }
